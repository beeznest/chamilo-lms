...
class Tracking
{
	public static function get_exercise_progress(
		...
		if (!empty($sessionId) && empty($courseId)) {
			...
			$sessions[$course->id][$sessionId] = Section::find(1)->toArray();
		}
        ...
        foreach ($courses as $courseIdx => $courseData) {
			...
			$userIds = array_combine(
				$result->lists('user_id'), 
				$result->lists('user_id')
			);

			$questionIds = array_combine(
				$result->lists('question_id'), 
				$result->lists('question_id')
			);
			...
			$resultQuestions = QuizQuestion::select(
				...
				->where('c_quiz_question.c_id', '=', $courseIdx)
				->whereIn('c_quiz_question.id', $result->lists('question_id'))
				// Fecth data
				->get();
			...
			$resultUsers = User::select(
				...
				->whereIn('user_id', $result->lists('user_id'))
				->get();
...
/* Location: .//var/www/html/chamilo-lms-utp/dev/components/Tracking.php */